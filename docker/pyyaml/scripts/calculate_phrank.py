...
class Phenotyper:
    ...
    def canonicalize_annotations(
        term_to_parents, gene_to_annotations, term_to_annotations
    ):
    ...
    @staticmethod
    def compute_term_info_content(
        hpoterms, term_to_parents, gene_to_annotations, term_to_annotations
    ):
        ...
        for term in hpoterms.values():
            ...
            termRawIc[term.id] = (
                -math.log(len(annotations) / annotatedGeneCt) / math.log(2)
                if len(annotations)
                else 0
            )
        ...
        for term in hpoterms.values():
            if term.id not in term_to_annotations:
                ...
                if len(parentTerms) == 0:
                    ...
                    parentGeneSets = [
                        set([a.gene for g, a in term_to_annotations[t].items()])
                        for t in parentTerms
                    ]
                    parentIntersection = functools.reduce(
                        lambda a, b: a.intersection(b), parentGeneSets
                    )
                    parentIc = -math.log(
                        len(parentIntersection) / annotatedGeneCt
                    ) / math.log(2)
    ...
    def __init__(self, termsfile, dagfile, annotationsfile):
        ...
        f.close()
        Phenotyper.canonicalize_annotations(
            self._buHpoDag, self._hpoGeneToAnnotations, self._hpoTermToAnnotations
        )

        self._termMarginalIc = Phenotyper.compute_term_info_content(
            self._hpoTerms,
            self._buHpoDag,
            self._hpoGeneToAnnotations,
            self._hpoTermToAnnotations,
        )
    ...
    def _phenotype_information(self, term_list):
        ...
        return sum(
            [
                self._termMarginalIc.get(t, 0)
                for t in self.term_ancestor_closure(term_list)
            ]
        )
    ...
    def phenotype_score(self, term_list1, term_list2):
        ...
        the two term lists."""
        return self._phenotype_information(
            self.term_ancestor_closure(term_list1).intersection(
                self.term_ancestor_closure(term_list2)
            )
        )
    ...
    def all_gene_scores(self, term_list=None, facet_by_condition=False):
        ...
        for g, annotations in self._hpoGeneToAnnotations.items():
            ...
            for facet, facet_terms in gene_facet_terms.items():
                (g, s) = facet
                gene_facet_scores.setdefault(s, {})[g] = self.phenotype_score(
                    term_list if term_list is not None else facet_terms, facet_terms
                )
...
def main(args):
    phenotyper = Phenotyper(
        args.hpo_terms_tsv, args.hpo_dag_tsv, args.ensembl_to_hpo_tsv
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "hpo_dag_tsv", help="HPO DAG structure (child to parent)", type=str
    )
    parser.add_argument(
        "ensembl_to_hpo_tsv", help="Map from Ensembl genes to HPO terms", type=str
    )
    parser.add_argument(
        "ensembl_to_hgnc", help="Map from Ensembl gene ID to HGNC gene symbol", type=str
    )
    ...
    parser.add_argument(
        "phrank_out_tsv",
        help="Phrank scores: <gene_symbol><TAB><phrank_score>",
        type=str,
    )
