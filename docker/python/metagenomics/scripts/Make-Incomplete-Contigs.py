...
##    Change passed_bins TSV format to have columns [bin, contig] (as output by Fasta-Make-Long-Seq-Bins.py); previously columns [contig, bin]
##    Make outdir optional; if it is provided, maintain script's previous behaviour and copy passed long bins to this directory
...
def get_args():
    ...
    parser.add_argument(
        "-i", "--input_fasta", required=True, help="The input contigs fasta file."
    )
    parser.add_argument(
        "-p",
        "--passed_bins",
    ...
    parser.add_argument(
        "-f", "--output_fasta", required=True, help="Name of output contigs fasta file."
    )
    parser.add_argument(
        "-d",
        "--fastadir",
    ...
    parser.add_argument(
        ...
        "--outdir",
        required=False,
        help="Name of output directory to copy passed long bins. If provided, complete contigs will be copied to this directory.",
    )
...
def get_complete_bins(passed_bins):
    """
    complete.1  s4.ctg000005c
    complete.3  s8.ctg000009c
    complete.4  s9.ctg000010c
    """
    print("get_complete_bins: Gathering names of complete contigs.")
    with open(passed_bins, "r") as fh:
        complete_bin_names = [line.split("\t")[0] for line in fh]
    with open(passed_bins, "r") as fh:
        complete_contigs = [line.strip().split("\t")[1] for line in fh]
...
def write_output_contig_fasta(input_fasta, complete_contigs, output_fasta):
    ...
    print(
        "write_output_contig_fasta: Found {:,} contigs in input fasta file.".format(
            recs
        )
    )
    print(
        "write_output_contig_fasta: Wrote {:,} contigs to output fasta file.".format(
            incomplete_count
        )
    )
...
def write_long_bins(fastadir, outdir, complete_bin_names):
    ...
    if complete_bin_names:
        ...
        print(
            "write_long_bins: No long bin files passed filters, skipping copy.".format(
                bin_count
            )
        )
