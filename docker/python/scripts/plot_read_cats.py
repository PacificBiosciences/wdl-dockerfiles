...
limaReport = pd.read_csv(
    snakemake.input.lima, sep="\t", usecols=["ReadLengths", "PassedFilters"]
)
...
allData.fillna("noSampleData").groupby(
    ["category", "sample"]
).length.describe().dropna().astype(int).to_csv(f"{outdir}/read_length_by_sample.csv")
